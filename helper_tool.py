...
class ConfigSSTz:
    ...
    num_classes = 1  # Number of valid classes
    sub_grid_size = 0.06  # preprocess_parameter
...
class DataProcessing:
    ...
    def load_label_kitti(label_path, remap_lut):
        ...
        se_label = label & 0xFFFF  # semantic label in lower half
        inst_label = label >> 16  # instance id in upper half
        assert ((se_label + (inst_label << 16) == label).all())
        se_label = remap_lut[se_label]
        return se_label.astype(np.int32)

    def load_label_sst(label_path, remap_lut):
        label = np.fromfile(label_path, dtype=np.uint32)
        label = label.reshape((-1))
    ...
    def get_file_list(dataset_path, test_scan_num):
        ...
        for seq_id in seq_list:
            ...
            if seq_id == '08':
                ...
                test_file_list.append([join(pc_path, f) for f in np.sort(os.listdir(pc_path))])
            elif seq_id in ['01','03','04','05','06','07','09']:
    ...
    def IoU_from_confusions(confusions):
        ...
        # confusion matrix), and that the last axis counts the labels (like the second axis of a confusion matrix)
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name is 'S3DIS':
            ...
            num_per_class = np.array([620217598, 2175486])
        elif dataset_name is 'SSTz':
            num_per_class = np.array([1002977])
