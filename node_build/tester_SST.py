...
data_config = join(BASE_DIR, 'utils', 'sst.yaml')
...
class ModelTester:
    ...
    def test(self, model, dataset):
        ...
        save_path = join(test_path, dataset.test_scan_number, 'labels')
                ...
                if np.min(dataset.min_possibility) > 0.5:  # 0.5
                    ...
                    for j in range(len(self.test_probs)):
                        ...
                        if dataset.test_scan_number == '08':
                            label_path = join(dirname(dataset.dataset_path), 'sequence_0.06', dataset.test_scan_number,
                                              'labels')
                            label_file = join(label_path, str(frame) + '.label')
                            labels = DP.load_label_sst(label_file, remap_lut_val)
                            invalid_idx = np.where(labels == -1)[0]  # 라벨값이 -1인 경우를 삭제할 수 있도록 invalid_idx 수정
                            ...
                            store_path = join(test_path, dataset.test_scan_number, 'labels',
                                              str(frame) + '.npy')
                            pred = pred + 1
