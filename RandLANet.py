...
def batch_gather(params, indices):
    ...
    batch_size = params_shape[0]
    num_indices = indices_shape[1]
    flattened_indices = tf.reshape(indices, [-1])
    flattened_range = tf.range(0, batch_size) * params_shape[1]
    flattened_indices = flattened_indices + flattened_range
    flattened_params = tf.reshape(params, [-1, params_shape[2]])
    gathered_params = tf.gather(flattened_params, flattened_indices)
    return tf.reshape(gathered_params, [batch_size, num_indices, params_shape[2]])
...
class Network:
    ...
    def get_loss(self, logits, labels, pre_cal_weights, alpha=1.0, gamma=3.0): #no binary focal loss
        class_weights = tf.convert_to_tensor(pre_cal_weights, dtype=tf.float32)
        one_hot_labels = tf.one_hot(labels, depth=self.config.num_classes)
        weights = tf.reduce_sum(class_weights * one_hot_labels, axis=1)
        prob = tf.nn.softmax(logits)
        pt = tf.reduce_sum(prob * one_hot_labels, axis=1)
        focal_weights = alpha * tf.pow(1 - pt, gamma)
        unweighted_losses = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=one_hot_labels)
        weighted_losses = unweighted_losses * focal_weights * weights
        output_loss = tf.reduce_mean(weighted_losses)
        return output_loss
    ...
    def nearest_interpolation(feature, interp_idx):
        ...
        interpolated_features = batch_gather(feature, interp_idx)
    ...
    def gather_neighbour(pc, neighbor_idx):
        ...
        features = batch_gather(pc, index_input)
